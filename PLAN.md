...
- [-] **iOS 스킴 분리** — ❌**일부러 안 한다**(§15). 한 기기에 학생·교사를 같이 깔기 위한 것인데 **교실에서는 기기가 다르다.** Xcode 프로젝트 편집은 빌드를 깨뜨리기 쉽고(09-10 에 파드 하나 바꾸다 흰 화면을 두 번 봤다) 이득은 개발 편의뿐이다. Android 는 이미 flavor 로 갈려 있다(`student`/`teacher`)
...
- [x] TTS 문제 읽기 (2026-09-10, `DictationSpeaker` — 문항별 🔊 + 속도 3단계.
      ⚠️`speechRate` 는 플랫폼마다 척도가 달라 우리 척도 0~1 을 변환한다)
- [x] 오답 노트 (2026-09-10, `MistakeAnalysis` — 취약 자모를 **그대로 가리기 학습지로**
      넘긴다. ⚠️채점이 음절 단위라 근사이고, 화면에서 그렇게 밝힌다)
- [x] 교사: jammin 스타일 **자모 가리기 옵션** 출제 (2026-09-10, `HideRule` +
      `HideRulePicker`. 웹 TS 구현과 **동치임을 테스트로 강제**한다)
...
- [~] 손글씨 → 텍스트 인식 — **2026-09-16 기본 구현까지.** 수단은 **ML Kit Digital Ink 양 플랫폼**으로 정했고(🔴09-07 에 걷어낸 것은 `google_mlkit_text_recognition`=**이미지 OCR** 이었다 — 이 앱은 이미 **획 좌표**를 받으므로 필요한 건 Digital Ink 다. 한국어 `ko` 지원), **좌표를 다루는 부분과 계약까지** 만들었다: `HandwritingInk`·`InkBuilder`·`HandwritingRecognizer`(19 GREEN). 🔴**네이티브는 교실 2대 리허설 뒤에 꽂는다**(사용자 결정) — 붙이는 순간 iOS 시뮬레이터 빌드가 깨지는데 실기기가 한 대뿐이라 그 리허설을 **시뮬레이터 허브로** 치러야 한다(§204). PLAN §231 이 적어 둔 *"시뮬레이터 검증을 포기하지 않는 대안을 먼저 따질 것"* 을 이렇게 지킨다. ⚠️씨앗에 외부 패키지가 못 들어오게 **테스트가 잠갔다**
- [x] 인식 후처리 = **답안 정규화** (2026-09-10, `normalizeAnswer` — 조합 중 자모·
      전각·공백. ⚠️맞춤법은 고치지 않는다. 인식 수단과 무관하게 지금도 채점을 고친다)
...
- [~] 획순 **지도**는 넣었다(2026-09-10, `StrokeOrderCheck` — 획수·방향·순서를
      좌표로 판정해 지적). **애니메이션은 안 넣었다** — 자산의 선분 방향이
      교육적 방향과 반대일 수 있어(§14) 움직임으로 보여 주면 틀린 것을 가르친다
- [x] 경로 vs 획순 비교 (2026-09-10, `StrokeOrderCheck`). ⚠️글씨를 잘 썼는지는
      보지 않는다 — 좌표로 확실한 것(획수·방향·순서)만. 대각선은 판정 안 함

**2.4 (선택) AI**

- [x] 교사 기기만: API 키 Secure Storage + 명시적 동의 UI (2026-09-10, `AiConsent`).
      **기능보다 먼저** 만들었다 — 나중에 붙이면 "일단 되게" 하려고 평문에 두기 쉽다
- [~] AI 출제·글씨 교정 — ✅**2026-09-16 출제 구현**(`AiComposeService`, `osh-ai-gateway` 경유). 🔴세 겹으로 막는다: 허브 역할 + 명시적 동의 + 주소·키. **보내는 것은 학년·주제뿐**이고 답안·이름·기기 ID 는 이 경로에 들어올 자리가 없다(구조로 강제·테스트로 고정). ⚠️모델이 무엇을 주든 `DictationComposer.isComposable` 을 통과한 것만 남긴다 — 말로 시켰다고 지켜지지 않는다. ⚠️**글씨 교정은 손글씨 인식이 선행**이라 아직 아니다 (이미지 전송 경로는 그때 다시 판단한다)
- [x] 학생 단말 **외부 전송 없음** (2026-09-10). 🔴학생 화면에 남아 있던 jammin
      호출 버튼을 역할로 가렸고, `offline_first_test.dart` 가 HTTP 사용 파일을
      **전수로 고정**해 새로 생기면 깨진다
...
- [x] 학급/세션 개념 (2026-09-10, `ClassroomBoard` — 교사 기기가 **본 것**으로 명단을 만든다)
- [x] 학생 목록 (2026-09-10, `student.join` — 접속 직후 등록.
      ★제출로만 명단을 만들면 **한 문제도 안 푼 학생**이 안 보인다)
- [x] 실시간 응시 현황·진행률 (2026-09-10 — **덜 낸 학생이 위**로 정렬)
- [x] 학급 통계: 평균 정답률·취약 자모 (2026-09-10)

**3.2 동기화 확장**

- [x] 오프라인 문제 번들 (2026-09-10, `OfflineBundle` — Wi-Fi 가 단말 간 통신을
      막을 때의 우회로. ⚠️파일 자체가 열쇠라 **출제 방향으로만** 흐른다)
- [x] 재연결·ACK·재전송 (2026-09-10 — `ack` 는 타입만 있고 안 쓰여서
      **제출이 조용히 사라지는데 화면은 "제출됨"** 이었다)
- [-] (장기) Nearby / Multipeer 어댑터 — ❌**2026-09-16 안 하기로 결정**(§15). 교실에 Wi-Fi 가 있다고 전제한다

**3.3 부모 모드**

- [x] 자녀 기기 연결 — ✅**2026-09-16 구현**. `AppRole.parent` + `main_parent.dart`. 교사 앱과 **같은 허브 기계**를 쓰되 학교 것이 빠진다: 🔴**서버 업싱크 UI 를 통째로 숨기고**(`canUpsync` — 집 기기에 학급 토큰을 넣게 만들면 자녀 답안이 학교 서버로 나간다) 라벨이 "학급"→"자녀". 역할 규칙을 `AppRoleRules` 한 곳에 모았다 — 화면마다 `teacher || parent` 를 적으면 언젠가 한 곳이 빠진다. 테스트 16(규칙 11 + 배선 5), 변이 3종 확인
- [x] 학습 요약 (2026-09-10, `LearningSummary`). 🔴표본이 적으면 추세를
      **말하지 않는다** · ⚠️등수·비교를 만들지 않는다

---

### Phase 4 — 운영·확장

- [x] 오프라인 핵심 자산 (2026-09-10) — **이미 번들이라 캐시가 불필요**했다.
      필요한 것은 그 사실의 증명이었고 `offline_first_test.dart` 가 본다
- [x] PDF 학습지 출력 (2026-09-10, `WorksheetPdf` — **의존성 없이** 직접 쓴다.
      플러그인을 늘리면 흰 화면 함정이 재발한다. 판정은 macOS Quick Look 으로)
- [ ] Play Store / App Store 제출
- [ ] 교육용 태블릿 MDM·웨일북 대응 검증
- [-] (선택) jammin 웹 API — ❌**일부러 안 한다**(§15). 09-07 에 **일부러 뗀 것**이다(출제가 원격 서버에 묶여 통째로 실패했다 — "중앙 서버 없음" 전제 위반). 되살릴 이유가 없다
...
- [x] jammin 실서버 대조 (2026-09-10, `chominjungum-web/golden/verify-against-jammin.mjs`
      — **실측 22/22 일치**. 골든이 스냅샷이라 원본이 바뀌면 셋이 같이 틀린다)
...
✅ **2026-09-16 결정: 3번(현재 설계) 유지.** Hive 큐가 보존하므로 그대로 동작하고 **코드 변경이 0** 이다.
같은 날 정한 초민정음웹 방향(**교사 개인 도구**)과도 맞는다 — 교실에서 실시간으로 올릴 이유가 없다.
⚠️ 1번(앱에 게이트 비번)은 **채택하지 않는다**. 2번(nginx 예외)은 필요해지면 그때 pm2 에 요청한다
(nginx 는 공유 자원이라 단일 소유자가 관리한다).
...
## 15. 2026-09-10 — 로드맵 소진, 그리고 **안 한 9건에 대한 판단**

하루에 로드맵 26건 중 17건을 넣었다(§14 이후 커밋 13건). 남은 9건은(+09-11 에 1건 추가) **못 한 것이
아니라 지금 하면 안 되는 것**이라 이유를 적어 둔다 — 안 그러면 다음 사람이 같은
고민을 처음부터 다시 한다.

### ✅ 2026-09-16 — 사람이 정했다 (5건 전부)

| 항목 | 결정 | 근거 |
|---|---|---|
| **손글씨 인식** | ✅ **ML Kit Digital Ink 양 플랫폼** | 🔴**전제가 틀렸었다** — 09-07 에 걷어낸 것은 `google_mlkit_text_recognition`(**이미지 OCR**)이고, 이 앱은 이미 **획 좌표**를 받는다(`stroke_canvas`·`glyph_strokes`). 필요한 것은 OCR 이 아니라 **Digital Ink** 다. 한국어 `ko` 지원 확인. ⚠️대가 = **iOS 시뮬레이터 빌드 불가**(arm64 슬라이스 부재는 Digital Ink 만이 아니라 `GoogleMLKit`·`MLKitCommon`·`MLKitVision` **파드 전체**·iOS 26+). 실기기·Android 는 무관 |
| **AI 출제·글씨 교정** | ✅ **`osh-ai-gateway` 경유** | 홈랩 표준 경로. 키가 게이트웨이에만 있고 비용·모델을 한 곳에서 갈아끼운다. `AiConsent` 는 이미 있음 |
| **부모 모드** | ✅ **집에서 부모 기기가 허브** | 교사 앱과 같은 구조를 집에서 쓰는 것. **오프라인 번들로 이미 된다** — 서버도 새 코드도 거의 불요하고 "중앙 서버 없음" 전제가 유지된다 |
| **Nearby / Multipeer** | ❌ **안 한다** | 교실에 Wi-Fi 가 있다고 전제한다 |
| **번들 QR 슬림화** | ❌ **안 한다** | 프로토콜 변경이라 교사·학생 앱을 동시에 올려야 하는데, **지금도 파일·클립보드로 번들을 주고받고 QR 화면 자체가 없어** 사용자에게 깨지는 것이 없다 |

⚠️ **손글씨를 켜면 iOS 시뮬레이터 통합테스트가 깨진다.** 이 저장소는 시뮬레이터에서
`integration_test` 를 돌려 왔으므로(`all_device_tests.dart` 는 실기기, 예행은 시뮬레이터),
**착수 전에 시뮬레이터 경로를 어떻게 할지 먼저 정해야 한다** — 조건부 의존성으로 뺄지,
실기기 전용으로 옮길지.

---

### ~~사람이 정해야 한다 (5건)~~ ← 위에서 전부 결정됨. 아래는 결정 당시의 선택지 기록

| 항목 | 무엇을 정해야 하나 |
|---|---|
| **손글씨 인식** | ML Kit 을 09-07 에 걷어냈다(시뮬레이터 arm64 슬라이스 부재). 후보는 **Apple Vision**(iOS 전용·온디바이스·무료) vs **서버 OCR**(양 플랫폼·외부 전송). 앱의 전제가 "중앙 서버 없음"이라 전자가 맞아 보이지만 **안드로이드가 비게 된다** |
| **AI 출제·글씨 교정** | 어느 모델인가(홈랩 `osh-ai-gateway` 경유? Gemini 직결?) · 비용은 누가 · 학부모 동의는 어디까지. ⚠️**동의·키 보관은 이미 만들어 뒀다**(`AiConsent`) — 모델만 정하면 붙는다 |
| **자녀 기기 연결(부모 모드)** | "부모"가 무엇인지가 안 정해졌다. 집에서 부모 기기가 허브가 되는 것인가, 교사가 중계하는 것인가. 전자면 오프라인 번들로 이미 되고, 후자면 서버가 필요하다 |
| **Nearby / Multipeer** | Wi-Fi 없는 교실을 위한 것인데, **그런 교실이 실재하는지** 확인이 먼저다. 리허설에서 Wi-Fi 격리가 문제였다면 오프라인 번들로 충분할 수 있다 |
| **번들 페이로드 슬림화** (2026-09-11 추가) | QR 로 문제를 나눠 줄 것인가. 자모를 빼면 10문항이 962B 로 QR 에 들어간다(지금 11974B). 학생 기기가 로컬로 다시 분해하고 **골든 벡터가 세 이식본의 동치를 강제**하므로 기술적으로는 안전하다. 다만 **프로토콜 변경**이라 `feat/b2-x25519` 와 하위호환을 함께 봐야 한다. ⚠️**지금 급하지 않다** — 번들 QR 화면 자체가 없고, 파일·클립보드 경로가 이미 동작한다 |

### 계정·하드웨어가 있어야 끝난다 (3건)

- **스토어 제출** — 개발자 계정·심사. 코드가 아니라 절차다
- **MDM·웨일북 검증** — 해당 기기가 있어야 한다
- **Android release 서명** — 키스토어는 **사람의 비밀**이라 내가 만들면 안 된다.
  ⚠️키를 잃으면 그 앱은 **영원히 업데이트할 수 없다** — 만드는 사람이 백업까지 책임져야 한다

### 일부러 안 한다 (2건)

- **iOS 스킴 분리** — 한 기기에 학생·교사를 같이 깔기 위한 것인데 **교실에서는
  기기가 다르다**. Xcode 프로젝트 편집은 빌드를 깨뜨리기 쉽고(오늘 파드 하나
  바꾸다 흰 화면을 두 번 봤다) 이득은 개발 편의뿐이다
- **jammin 웹 API 연동** — 09-07 에 **일부러 뗀 것**이다(출제가 원격 서버에 묶여
  통째로 실패했다). 되살릴 이유가 없다. 콘텐츠 가져오기가 필요하면
  `verify-against-jammin.mjs` 처럼 **개발 도구로** 두는 것이 맞다

### ⚠️ 오늘 넣은 것 중 실기기에서 안 태운 것

실기기 검증은 오늘 중간에 한 번 통과했다(TTS·획순 자산·가리기 왕복·오답 노트).
**그 뒤에 넣은 것**은 아직이다 — 학급 현황(`student.join`) · 오프라인 번들 ·
ACK 재전송 · PDF 출력 · 획순 지도 · AI 동의.

🔴**`flutter test integration_test` 가 아니라 `flutter drive` 를 쓸 것**
(`integration_test/README.md`). 같은 기기에서 전자는 2회 연속 실패했다.

> **2026-09-11 진행** — 위 여섯에 **통합 테스트 자체가 없던 것**을 메웠다
> (`integration_test/late_additions_device_test.dart`, 시뮬레이터 **7/7**).
> 여섯 다 호스트 테스트가 원리상 닿지 못하는 층이다(키체인·GPU 래스터·실제
> 소켓·플랫폼 암호·포인터 이벤트). **실기기 실행만 남았다** — 아이폰 연결 +
> 잠금 해제 후 `flutter drive`.
>
> 쓰면서 나온 것 둘:
> - **획은 칸 안에서만 받는다** — 학습지 전체 사각형으로 제스처를 잡으면 칸
>   밖에 떨어져 아무 일도 안 일어난다. "지도가 고장났다"로 오판하기 쉽다
> - **QR 경계는 3음절**(실측: 2음절 898B ○ / 4음절 1438B ✗ / 10문항 11974B ✗,
>   상한 1200B). 실제 받아쓰기는 QR 로 절대 안 된다. 무게의 70% 가
>   `expectedGlyphsJson` 이고 학생 기기가 로컬로 다시 분해할 수 있는 값이라
>   빼면 10문항이 962B — ~~프로토콜 변경이라 사람 결정으로 남긴다~~ → ✅**2026-09-16 안 하기로 결정**(§15).
>   지금도 파일·클립보드로 번들을 주고받고 **QR 화면 자체가 없어** 사용자에게 깨지는 것이 없다
