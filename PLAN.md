...
- [ ] 손글씨 → 텍스트 인식 — ✅**2026-09-16 수단 결정: ML Kit Digital Ink 양 플랫폼**(§15). 🔴09-07 에 걷어낸 것은 `google_mlkit_text_recognition`(**이미지 OCR**)이고 이 앱은 이미 **획 좌표**를 받으므로, 필요한 것은 OCR 이 아니라 Digital Ink 다. 한국어 `ko` 지원 확인. ⚠️착수 전에 **iOS 시뮬레이터 경로를 어떻게 할지** 먼저 정해야 한다(ML Kit 파드 전체가 arm64 시뮬 슬라이스가 없다)
